...
QImage DesktopWindow::loadWallpaperFile(QSize requiredSize) {
    ...
    if(useCache) {
        ...
        cacheFileName += QLatin1String("/lwqt-pcmanfm/") + app->profileName();
...
bool DesktopWindow::event(QEvent* event) {
    ...
    case QEvent::WinIdChange:
        resetWindowType = effectiveWinId();
        break;
    case QEvent::UpdateRequest:
        if (WId wid = effectiveWinId()) {
            ...
            if (info.valid()) {
                ...
                if (resetWindowType && KWindowInfo(wid, {}).valid()) {
                    resetWindowType = false;
                    KWindowSystem::setType(wid, NET::Desktop);
                    KWindowSystem::setState(wid, NET::Sticky | NET::KeepBelow);
                }
            }
        }
        break;
